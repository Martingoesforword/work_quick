...
private:
	...
	int control_index;
	void(*func)(control_struct, data_struct, int index);
	I_ScrollBar_Adv* scr;
public:
	ListUpdate(I_ScrollBar_Adv* scrs, vector< control_struct>& conss, vector<data_struct>& datass, void(*funcc)(control_struct, data_struct, index int))
	...
	void update()
	{
		...
		vector<data_struct>::iterator  pos = datas.begin();
